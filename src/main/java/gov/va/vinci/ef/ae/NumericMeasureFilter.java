...
public class NumericMeasureFilter extends LeoBaseAnnotator {
   ...
   */
  protected WindowService windowService = new WindowService(10, 5, Window.class.getCanonicalName());
    ...
    for (NumericValue value : values) {
      try {
        Window window = (Window) windowService.buildNonWhitespaceTokenWindow(value);
         ...
         // System.out.println("Value from numeric measure filter: " + value.getCoveredText());
         // System.out.println("Window from numeric measure filter: " + window.getCoveredText());
